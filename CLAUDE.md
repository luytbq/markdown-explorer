...
The guess limit is global, not per address, because behind a tunnel every request comes from loopback. auth.attempt checks the limit, compares, and counts a failure in one synchronous call, and handleLogin calls it only after the body has been read. Nothing may await between the check and the count: every request waiting at that await passes a check made before its own failure is counted, so a client that sends its headers, holds a thousand bodies back and releases them together gets a thousand guesses answered. That is why auth.js exports attempt and not the check and the compare separately. test/auth.test.js holds bodies back exactly that way; the green tests beside it send headers and body in one packet and pass with the race wide open. A limited attempt is neither compared nor counted, and the limit never looks at an existing session, so spending the budget locks out new sign-ins, never the owner's open tab.
